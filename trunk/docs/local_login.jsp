<%@page import="TOBECONFIG.nembo.nemboconf.util.NemboUtils"%>
<%@page import="TOBECONFIG.nembo.nemboconf.presentation.login.CtrlLogin"%>
<%@page import="TOBECONFIG.nembo.nemboconf.exception.*"%>
<%@page import="TOBECONFIG.nembo.nemboconf.dto.MapColonneNascosteVO"%>
<%@page import="TOBECONFIG.papua.papuaserv.dto.gestioneutenti.ws.UtenteAbilitazioni"%>
<%@page import="TOBECONFIG.papua.papuaserv.dto.gestioneutenti.UtenteLogin"%>
<%@page import="TOBECONFIG.papua.papuaserv.presentation.rest.profilazione.client.PapuaservProfilazioneServiceFactory"%>
<%@page import="TOBECONFIG.nembo.nemboconf.util.validator.Errors"%>
<%@page import="java.text.SimpleDateFormat"%>
<%@page import="java.util.Date"%>
<%@page import="java.util.HashMap"%>
<%@page import="java.util.Map"%>
<%@page import="TOBECONFIG.iride2.policy.entity.Identita"%>
<%@page import="java.util.ArrayList"%>
<%@page import="TOBECONFIG.nembo.nemboconf.dto.DecodificaDTO"%>
<%@page import="TOBECONFIG.nembo.nemboconf.util.NemboConstants"%>
...
<%!private static final String[]                         CODICI_FISCALI =
  { "AAAAAA00B77B000F", "AAAAAA00A11B000J", "AAAAAA00A11C000K", "AAAAAA00A11D000L", "AAAAAA00A11E000M",
      ...
      "AAAAAA00A11V000D", "AAAAAA00A11P000X" ,"SRSLSN79D64L219H"};
  ...
  static
  {
    ...
    {
      DEMO.add(new DecodificaDTO<String>(String.valueOf(i), "TOBECONFIG " + (i + 20) + " (" + CODICI_FISCALI[i] + ")"));
    }
  }
  ...
  {
    Cookie cookie = new Cookie(NemboConstants.PORTAL.NEMBOCONF_LOGIN_PORTAL, portal);
...
<%
  ...
  if (!isSubmit)
  {
    request.setAttribute("idxCodiceFiscale", "8");
  ...
  else
  {
        ...
        if (request.getParameter("loginID") != null)
        {
          ...
          if (errors.isEmpty())
          {
            ...
            if (listUtenti != null && listUtenti.length > 0)
            {
              ...
              if (utenteLogin.getIdProcedimento() == NemboConstants.NEMBOCONF.ID)
              {
                ...
                if ("ipa".equals(idProvider))
                {
                  ...
                  session.setAttribute(NemboConstants.PORTAL.NEMBOCONF_LOGIN_PORTAL, NemboConstants.PORTAL.PUBBLICA_AMMINISTRAZIONE);
                }
                else
                {
                  addPortalCookie(response, NemboConstants.PORTAL.PRIVATI);
                  session.setAttribute(NemboConstants.PORTAL.NEMBOCONF_LOGIN_PORTAL, NemboConstants.PORTAL.PRIVATI);
                }
                session.setAttribute("identita", identita);
                String nextPage = CtrlLogin.login(session, utenteLogin.getRuolo().getCodice());
                nextPage = nextPage.replaceAll("redirect:(\\b)*(../)*", "/nemboconf/");
                response.sendRedirect(nextPage);
                return;
              }
              else
              {
                errors.addError("error", "L'utente corrispondente a questo Id NON E' di nemboconf");
                errors.addError("idUtenteLogin", "L'utente corrispondente a questo Id NON E' di nemboconf");
              }
            }
            else
            {
    ...
    if (identita != null)
    {
      ...
      if ("ipa".equals(idProvider))
      {
        response.sendRedirect("/nemboconf/login/wrup/seleziona_ruolo.do");
      }
      else
      {
        response.sendRedirect("/nemboconf/login/sisp/seleziona_ruolo.do");
      }
      return;
    }
  }
