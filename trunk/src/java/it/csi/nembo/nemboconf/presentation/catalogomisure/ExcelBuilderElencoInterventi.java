package TOBECONFIG.nembo.nemboconf.presentation.catalogomisure;
...
public class ExcelBuilderElencoInterventi extends AbstractExcelView
{
      ...
      throws Exception
  {
    ...
    List<InterventiDTO> elenco = (List<InterventiDTO>) model.get("elenco");
    ...
    col++;
    header.createCell(col).setCellValue("Intervento");
    header.getCell(col).setCellStyle(style);
    sheet.addMergedRegion(new CellRangeAddress(0, 1, col, col));
    ...
    for (InterventiDTO item : elenco)
    {
      ...
      aRow.createCell(col).setCellValue(item.getDescTipoIntervento());
      aRow.getCell(col).setCellStyle(styleValDef);

      col++;
      aRow.createCell(col).setCellValue(item.getDescIntervento());
      aRow.getCell(col).setCellStyle(styleValDef);
