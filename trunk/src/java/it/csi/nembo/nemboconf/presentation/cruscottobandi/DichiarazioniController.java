...
public class DichiarazioniController extends BaseController
{
  ...
  @RequestMapping(value = "dichiarazioni", method = RequestMethod.GET)
      ...
      HttpSession session) throws InternalUnexpectedException
  {
    ...
    List<DecodificaDTO<String>> gruppiDisponibili = cruscottoEJB
        .getElencoGruppiControlliDisponibili(idBando, "DICH");
    ...
    return "cruscottobandi/dichiarazioni";
  }

  @SuppressWarnings("unchecked")
  @Security(dirittoAccessoMinimo = Security.DIRITTO_ACCESSO.READ_ONLY, macroCDU = Security.GESTISCI_CRUSCOTTO_BANDI)
  @RequestMapping(value = "visualizzadichiarazioni", method = RequestMethod.POST)
      ...
      throws InternalUnexpectedException
  {
    ...
    {
      List<DecodificaDTO<String>> vincoliTrovati = cruscottoEJB
          .getIdLegameInfo(infoBandoOggetto.getId(), idQuadroOggetto);
      prepareVincoli(vincoliTrovati, session);
    }

    // Controllo se sono già stati inseriti dei procedimenti per questo bando,
    // perchè in questo caso devo limitare le azioni sulla pagina
    session.setAttribute("sessionProcedimentoOggettoQuadroInserito",
        cruscottoEJB.isProcedimentoOggettoQuadroInserito(bando.getIdBando(),
  ...
  @SuppressWarnings("unchecked")
  @RequestMapping(value = "confermadichiarazioni", method = RequestMethod.POST)
      ...
      throws InternalUnexpectedException
  {
    ...
    if (dichiarazioni != null && bando != null)
    {
      ...
      if (errors.isEmpty())
      {
        ...
        logOperationGruppoInfoDTO("dichiarazione", idBandoOggetto, nomeQuadro,
            ...
            if (dettaglioDich == null && tentativiDett == 30)
            {
              ...
              else
              {
                    ...
                    && !GenericValidator.isBlankOrNull(vincolo))
                {
                  errors.addError(
                      "flagObblDetailGruppo_" + cIdDettaglio + "_" + cIdGruppo,
                      "Non è possibile inserire i vincoli su dichiarazioni obbligatorie");
                }
  ...
  @SuppressWarnings("unchecked")
  @RequestMapping(value = "avantidichiarazioni", method = RequestMethod.POST)
      ...
      HttpSession session) throws InternalUnexpectedException
  {
    ...
    if (gruppiDisponibili != null)
      for (DecodificaDTO<String> item : gruppiDisponibili)
      {
        ...
        {
          msgErrore = msgErrore
              + "Non hai inserito alcuna dichiarazione per l'oggetto/istanza "
              + item.getDescrizioneEstesa()
              + ", oppure non hai selezionato quelle obbligatorie. <br>";
        }
      }
    ...
    return "redirect:impegni.do";
  }

  @SuppressWarnings("unchecked")
  @RequestMapping(value = "refreshGruppoInfo", method = RequestMethod.POST)
...
//TODO: FIXME: NullPointerException
    ...
    validaDichiarazioni(model, request, session, idQuadroOggetto,
        idBandoOggetto, dichImportateCatalogo, new Errors());
