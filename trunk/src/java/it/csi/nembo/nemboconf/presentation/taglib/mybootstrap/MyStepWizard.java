...
/**
 *         Bisogna passare il numero di steps e una stringa con i nomi dei vari
 *         step separati da "&&&". L'attributo "activeStep" riceve un int e
 *         indica qual è lo step corrente. Il parametro "type" riceve "square"
 *         or "circle". E' facoltativo, di default è "circle".
 */
public class MyStepWizard extends MyBodyTag
{
      ...
      throws Exception
  {
    ...
    if (labelsArray.length != numberOfSteps.intValue())
      throw new Exception(
          "Il numero di step è diverso dal numero di argomenti passati.");
    ...
    for (int i = 1; i <= numberOfSteps; i++)
    {
      ...
      sb.append("<div class=\"stepwizard-step\" style=\"width: 100/"
          + numberOfSteps + "%;\">");
      if (activeStep.intValue() == i)
        sb.append("<a href=\"#step-" + i
            + "\" type=\"button\" class=\"btn btn-primary btn-" + type + "\">"
            + i + "</a>");
      else
        sb.append("<a href=\"#step-" + i
            + "\" type=\"button\" class=\"btn btn-default btn-" + type + "\">"
            + i + "</a>");
