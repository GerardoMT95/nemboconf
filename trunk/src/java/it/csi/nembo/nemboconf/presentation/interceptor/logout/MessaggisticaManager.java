package TOBECONFIG.nembo.nemboconf.presentation.interceptor.logout;
...
import TOBECONFIG.nembo.nemboconf.business.IPianoFinanziarioNemboEJB;
import TOBECONFIG.nembo.nemboconf.dto.messaggistica.InfoMessaggio;
import TOBECONFIG.nembo.nemboconf.integration.ws.papuaserv.messaggistica.IMessaggisticaWS;
...
import TOBECONFIG.nembo.nemboconf.integration.ws.papuaserv.messaggistica.Messaggio;
import TOBECONFIG.nembo.nemboconf.presentation.interceptor.BaseManager;
import TOBECONFIG.nembo.nemboconf.util.NemboConstants;
import TOBECONFIG.nembo.nemboconf.util.NemboUtils;
import TOBECONFIG.nembo.nemboconf.util.WsUtils;
...
public class MessaggisticaManager extends BaseManager
{
  ...
  static void loadMessaggistica()
  {
    try
    {
      ...
      IPianoFinanziarioNemboEJB ejb = (IPianoFinanziarioNemboEJB) ic
          .lookup("java:app/PianoFinanziarioNemboconf");
      Map<String, String> mapParametri = ejb.getParametriComune(
      ...
      HttpServletResponse response, Object handler) throws Exception
  {
    ...
    if (mapMessaggistica == null)
    {
       ...
       * RITORNA AL LOGIN!
       */
      response.sendRedirect("/" + NemboConstants.NEMBOCONF.WEB_CONTEXT
          + "/cunembo201/index.do");
      return false;
    }
    ...
    else
    {
      if (deltaTimestamp > MINUTI_VERIFICA_LOGOUT * 60 * 1000)
      {
        try
        {
          ...
          UtenteAbilitazioni utenteAbilitazioni = (UtenteAbilitazioni) getUtenteAbilitazioni(
              session);
          messaggisticaWS.verificaLogout(NemboConstants.NEMBOCONF.ID,
              utenteAbilitazioni.getRuolo().getCodice());
        }
        catch (LogoutException_Exception e)
        {
