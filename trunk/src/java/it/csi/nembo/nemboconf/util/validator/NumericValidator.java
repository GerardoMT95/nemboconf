package TOBECONFIG.nembo.nemboconf.util.validator;
...
public class NumericValidator extends TextValidator
{
  ...
  public boolean validateBigDecimalInRange(BigDecimal fieldValue,
      String fieldName, BigDecimal min, BigDecimal max, String msgAggiuntivo)
  {
    try
    {
      if (min != null && fieldValue.compareTo(min) < 0)
      {
        addError(fieldName, ERRORE_VALORE_MINIMO_AMMESSO
            + min.toString().replace(".", ",") + " " + msgAggiuntivo);
        return false;
      }
      else
      {
        if (max != null && fieldValue.compareTo(max) > 0)
        {
          addError(fieldName, ERRORE_VALORE_MASSIMO_AMMESSO
              + max.toString().replace(".", ",") + " " + msgAggiuntivo);
          return false;
        }
      }
      return true;
